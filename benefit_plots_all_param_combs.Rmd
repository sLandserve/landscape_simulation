...
for(i in 1:nrow(params)) {
  ...
  mod <- res %>% 
    ...
    preds <- mod %>% select(link_scenario, rival, mod_preds) %>% 
      unnest()
  
  p_title <- bquote(list(alpha==.(alpha_val), beta==.(beta_val), gamma==.(gamma_val)))
  
  plots[[i]] <- ggplot(preds, aes(x = pred, 
                                  y = benefit, 
                                  colour = modx_group, 
                                  linetype = rival)) + 
    geom_line() + 
    scale_color_discrete(name = "Amount Scenario") + 
    facet_grid(link_scenario ~ covariate, scales = "free")
}
```
