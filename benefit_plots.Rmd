...
Load results of the simulations (`sim_benefit.R`). We filter the results to only take benefit function values $\alpha = 1, \beta = 0, \gamma = 0.1$.  
...
out <- ldply(f, function(x) get(load(x))) %>% 
  ...
  summarise(num_supply_mean = mean(num_supply),
            ...
            ee_density_mean = mean(ee_density, na.rm=TRUE),
            ee_density_sd = sd(ee_density, na.rm=TRUE),
            es_density_mean = mean(es_density, na.rm=TRUE),
            es_density_sd = sd(es_density, na.rm=TRUE),
