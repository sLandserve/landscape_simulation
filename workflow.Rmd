...
p_supply <- p_demand <- seq(0.1, 0.5, by = 0.1)
...
# JONATHAN: I've no idea what I've done to break this bit of code, but I have. Sorry. 
net_plots <- lapply(net_first_rep, function(x) {net <- network(x$network$net_links,directed=FALSE); net %v% "Node Type" <- node_type; return(ggnet2(net, size = log(x$network$node_areas), max_size = 2, edge.size = 0.01, color = "Node Type", palette = "Set2") + guides(color = FALSE, size = FALSE) + ggtitle(paste("f demand =", x$params["f_demand"], "& f supply =", x$params["f_supply"], sep=" ")))})
